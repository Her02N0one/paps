...
func restore_persisted_people(area_path: String, current_level: BaseLevel) -> void:
	...
	# Avoid duplicate spawns by first indexing authored people already present in the level.
	...
	for person_id in people:
		...
		if person == null:
			...
			continue
		# Apply identity/profile/overrides before insertion so first _ready() sees final data.
		person.person_id = person_id
		person.apply_profile(record.get("profile", {}) as Dictionary)
		var instance_overrides: Variant = record.get("instance_overrides", {})
		if instance_overrides is Dictionary:
			SCENE_INSTANCE_OVERRIDES.apply(person, instance_overrides as Dictionary)
