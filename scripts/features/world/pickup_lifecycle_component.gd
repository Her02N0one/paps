...
func configure(game_state: GameState, save_manager: SaveManager, entity_root: Node3D, pickup_scene: PackedScene) -> void:
...
func restore_dynamic_pickups(area_path: String) -> void:
	...
	for dynamic_id in dynamic_pickups:
		...
		if item == null or typeof(saved_transform) != TYPE_TRANSFORM3D:
			...
			continue
		var instance_overrides: Variant = record.get("instance_overrides", {})
		spawn_dynamic_pickup(
			item,
			int(record.get("quantity", 1)),
			dynamic_id,
			saved_transform as Transform3D,
			instance_overrides if instance_overrides is Dictionary else {}
		)
...
func spawn_dynamic_pickup(
	...
	SCENE_INSTANCE_OVERRIDES.apply(pickup, instance_overrides)
