...
func _setup_internal_controllers() -> void:
	...
	if active_registry:
		var panels_layer = get_node("GamePanelsLayer")
...
func _bind_and_register_panel(panel: GamePanel) -> void:
	...
	if panel.has_signal("quit_to_menu_pressed") and not panel.quit_to_menu_pressed.is_connected(self._emit_quit_to_menu):
		panel.quit_to_menu_pressed.connect(self._emit_quit_to_menu)
	if panel.has_signal("quit_game_pressed") and not panel.quit_game_pressed.is_connected(self._emit_quit_game):
		panel.quit_game_pressed.connect(self._emit_quit_game)
