...
func start_new_game(initial_map: String) -> void:
	_enqueue_transition_request(
		&"start_new_game",
		Callable(self, "_perform_start_new_game").bind(initial_map)
	)
...
func _start_world(initial_map: String) -> void:
	...
	pending_target_spawn_id = NEW_GAME_ENTRY_ID if _show_intro_on_world_start else ""
...
func _perform_start_new_game(initial_map: String) -> void:
