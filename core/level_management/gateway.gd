...
func _get_available_movement(body: Node3D) -> ActorMovementSystem:
	# Only character bodies are eligible for movement-based travel requests.
	if not body is CharacterBody3D:
		return null
	return ActorMovementSystem.find_on(body)


func _is_reverse_entry(movement: ActorMovementSystem) -> bool:
