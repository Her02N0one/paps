...
const PICKUP_SCENE := preload("res://content/props/search_object.tscn")
...
func _run() -> void:
	...
	var dynamic_id := ServiceRegistry.pickup_manager.add_dynamic_pickup(area_path, SCRAP, 2, Transform3D(Basis.IDENTITY, Vector3(2.0, 1.0, 3.0)))

	component.restore_dynamic_pickups(area_path)
	var spawned := entity_root.get_child_count() == 1
	var restored_pickup := entity_root.get_child(0) as SearchObject
	var restored_quantity_ok := restored_pickup != null and restored_pickup.inventory_holder != null and restored_pickup.inventory_holder.inventory.get_quantity(SCRAP) == 2

	component.handle_pickup_collected("", dynamic_id)
	var removed_from_state := not ServiceRegistry.pickup_manager.get_dynamic_pickups(area_path).has(dynamic_id)
