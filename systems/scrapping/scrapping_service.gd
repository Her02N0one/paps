...
static func can_scrap(slot: InventorySlot, scrap_item: ItemData) -> bool:
	# Scrap item itself is excluded to prevent self-referential conversion loops.
	return slot != null and slot.data != scrap_item and slot.data.scrap_yield > 0


static func get_output_quantity(slot: InventorySlot, quantity: int) -> int:
	if slot == null or quantity <= 0:
		return 0
	# Clamp to slot quantity so callers can pass optimistic values safely.
	return slot.data.scrap_yield * mini(quantity, slot.quantity)


static func scrap_slot(inventory: InventoryStore, slot_id: int, quantity: int, scrap_item: ItemData) -> bool:
