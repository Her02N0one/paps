...
static func can_scrap(slot: ItemStack, scrap_item: ItemDefinition) -> bool:
	...
	return slot != null and slot.instance.definition != scrap_item and slot.instance.definition.scrap_yield > 0
...
static func get_output_quantity(slot: ItemStack, quantity: int) -> int:
	...
	return slot.instance.definition.scrap_yield * mini(quantity, slot.instance.quantity)
...
static func scrap_slot(inventory: InventoryStore, slot_id: int, quantity: int, scrap_item: ItemDefinition) -> bool:
	...
	if not can_scrap(slot, scrap_item) or quantity <= 0 or quantity > slot.instance.quantity:
		return false
