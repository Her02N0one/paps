...
func _show_selection(slot: ItemStack) -> void:
	...
	selected_name.text = slot.instance.definition.display_name if has_selection else "No scrappable item selected"
	selected_icon.texture = (slot.instance.definition.icon if slot.instance.definition.icon else FALLBACK_ICON) if has_selection else null
	selected_icon.visible = has_selection
	quantity_input.editable = has_selection
	quantity_input.max_value = slot.instance.quantity if has_selection else 1
	quantity_input.value = clampi(int(quantity_input.value), 1, slot.instance.quantity) if has_selection else 1
...
func _update_preview() -> void:
	...
	yield_label.text = "%d Scrap each" % slot.instance.definition.scrap_yield if has_output else "Select an item to dismantle"
