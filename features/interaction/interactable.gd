...
func _ready() -> void:
	...
	if collision_object:
		# Add the interaction layer (bit 2) without destroying existing physics layers (like layer 1).
		collision_object.collision_layer |= 2
		# We absolutely CANNOT set collision_mask to 0, because RigidBody3Ds need it to collide with the floor!
...
# sends player interaction request to UI controller
