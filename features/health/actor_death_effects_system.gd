...
class_name ActorDeathEffectsSystem
extends Node

const DEATH_EFFECT_PROFILE_SCRIPT := preload("res://features/health/actor_death_effect_profile.gd")
...
func _ready() -> void:
	...
	if _health_component == null:
		push_warning("ActorDeathEffectsSystem could not find ActorHealthComponent at '%s'." % String(health_component_path))
		return
...
func _resolve_effect_parent(profile: ActorDeathEffectProfile) -> Node:
	match profile.parent_mode:
		...
		ActorDeathEffectProfile.ParentMode.WORLD_EFFECT_ROOT:
			var wrld = ServiceRegistry.world
			if is_instance_valid(wrld):
				var effect_root := wrld.get_node_or_null("GameplayWorld/EffectRoot")
				if effect_root != null:
					return effect_root
