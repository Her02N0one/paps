...
const PERSON_VISUAL_RIG_COMPONENT_SCRIPT := preload("res://entities/characters/base_npc/person_visual_rig_component.gd")
const PERSON_PERSISTENCE_COMPONENT_SCRIPT := preload("res://entities/characters/base_npc/person_persistence_component.gd")
...
@onready var movement_component: ActorMovementComponent = get_node_or_null("ActorMovementComponent") as ActorMovementComponent
@onready var health_component: ActorHealthComponent = get_node_or_null("ActorHealthComponent") as ActorHealthComponent
@onready var visual_rig_component: PersonVisualRigComponent = get_node_or_null("PersonVisualRigComponent") as PersonVisualRigComponent
@onready var persistence_component: PersonPersistenceComponent = get_node_or_null("PersonPersistenceComponent") as PersonPersistenceComponent
...
func _ready() -> void:
	if visual_rig_component == null:
		push_error("PersonActor '%s' is missing a PersonVisualRigComponent." % name)
	if persistence_component == null:
		push_error("PersonActor '%s' is missing a PersonPersistenceComponent." % name)
	_connect_definition_changed(_definition)
