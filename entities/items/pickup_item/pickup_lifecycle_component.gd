...
func drop_item_from_player(definition: ItemDefinition, quantity: int, player: CharacterBody3D) -> void:
	...
	var dynamic_id := _game_state.add_dynamic_pickup(_game_state.current_area, definition, quantity, drop_transform)
	spawn_dynamic_pickup(definition, quantity, dynamic_id, drop_transform)
